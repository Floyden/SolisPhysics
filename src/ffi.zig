...
export fn PhysicsWorld_Create() *PhysicsWorld {
...
export fn PhysicsWorld_Destroy(world: *PhysicsWorld) void {
    world.deinit();
    std.heap.page_allocator.destroy(world);
}

export fn PhysicsWorld_Step(world: *PhysicsWorld, dt: f32) void {
    world.step(dt) catch unreachable;
}

export fn PhysicsWorld_CreateRectangleCollider(world: *PhysicsWorld, width: f32, height: f32, isometry: Isometry2) u64 {
    return world.addCollider(Collider2.new(CollisionShapes.newRectangle(width, height), isometry));
}

export fn Colliders_GetIsometry(world: *PhysicsWorld, collider: u64) *Isometry2 {
    return &world.getCollider(collider).transform;
}

export fn PhysicsWorld_CreateRigidBody(world: *PhysicsWorld, collider: u64, mass: f32) u64 {
